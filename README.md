 ...
 * Technical choices : - Dynamic force calculation
   ...
   - Lerp (linear interpolation) used to visually simulate to the bowstring tension

* Huge Problem : Control the projectile and the shoot of the bow
The Target and Chest Loop was a bit difficult to control at the beginning


## 2. Plateform 
An agillity course featurinf unpredictable, physics-driven obstacles
...
* Huge Problem :
 at first i tried with just a timeline, but not worked
With angular prameters and a little calculate of sinus on pitch(y) that fine !
 ...
 * Technical choices : - Grab system instead of simply attaching the cube to the hand, i usesd physics handle. This allow the cube to still collide with walls and react to the environment
   while being carried.
   - Pressure Plates :  Detection is handles via OnComponentBeginOverlap. 

* Huge Problem :
  Not very had a huge problem on this game, but at first i forgot to check simulate physic, that allow the player to press the button but not the cube.....
